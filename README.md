...
- **Annotations don't squeeze the document**: documents often cap their own width (e.g. `section { max-width: 1024px }` in the template's CSS) and size tables to fill it, and the annotation boxes' borders and padding would otherwise eat into that budget. The preview measures how much width the annotations add at the deepest nesting and widens any px `width`/`max-width` the document's CSS sets on `section` elements by exactly that amount — in the preview and in the standalone export — so the content keeps the full width it has in the finished page. Data-value chips also wrap inside table cells so they don't inflate column widths.
